...
**1つのターミナルから、複数ワーカーのHTTP負荷試験を動かす。**

[まず動かす](#まず動かす) · [JSONを送る](#json-を送る) · [自動実行](#1-回実行して結果を保存する) · [English](./README.md)

![3台のワーカーが合計180件を完了したSwarmGoの実行画面](assets/dashboard.svg)

*実際のローカル実行画面です。3台 × 60件を、応答に待ち時間を入れたサーバーへ送っています。性能の上限を測った結果ではありません。*

| 負荷を送る | 結果を見る | 自動で実行する |
| --- | --- | --- |
| GET・POST、本文・ヘッダー指定 | 進捗・処理量・エラーを画面で確認 | 1回実行し、JSONと終了コードを返す |

Goで実装し、コントローラーとワーカーはgRPCで通信します。下のComposeデモで、ワーカー3台とテスト対象をまとめて起動できます。
