...
plt.rcParams.update({'font.family':'Mona Sans','svg.fonttype':'path','svg.hashsalt':'swarmgo-repeat-six','font.size':13})
palettes={
    'light':dict(background='#ffffff',ink='#243d49',muted='#576973',grid='#e6ecef',
                 accent='#008caa',label='#00718a',neutral='#7c8d99',point='#536b78'),
    'dark':dict(background='#0d1117',ink='#f0f6fc',muted='#a0aab6',grid='#27303a',
                accent='#278ba0',label='#63d2e5',neutral='#657888',point='#bcc8d3'),
}
...
def style(ax):
    ...
    ax.set_yticks(range(len(tools)),names,fontsize=16)
    ax.get_yticklabels()[1].set_fontweight('semibold');ax.get_yticklabels()[1].set_color(palette['label'])
...
for theme,palette in palettes.items():
    ...
    fig,ax=plt.subplots(figsize=(8.4,4.3),facecolor=background)
    fig.text(.04,.91,'HTTP POST throughput',fontsize=20,fontweight='semibold',color=ink)
    ...
    for y,t in enumerate(tools):
        ...
        ax.text(1.19,y,f'{median:.0f}k',transform=ax.get_yaxis_transform(),ha='right',va='center',fontsize=20,fontweight='semibold' if t=='swarmgo' else 'normal',color=palette['label'] if t=='swarmgo' else ink,clip_on=False)
    style(ax)
    ax.set_xlim(0,max_x)
    ...
    fig,ax=plt.subplots(figsize=(8.4,5.3),facecolor=background)
    fig.text(.04,.91,'Run-to-run variation',fontsize=20,fontweight='semibold',color=ink)
    ...
    for y,t in enumerate(tools):
        ...
        ax.boxplot([v],positions=[y-.16],vert=False,widths=.22,whis=(0,100),showfliers=False,patch_artist=True,manage_ticks=False,
            boxprops={'facecolor':to_rgba(colors[t],.35),'edgecolor':ink},medianprops={'color':ink,'linewidth':2},
        ...
        for r in rows:
            ...
            ax.scatter(x,y+.13+lane*lane_step,s=28,facecolors=colors[t] if r['batch']%2==1 else background,edgecolors=palette['label'] if t=='swarmgo' else palette['point'],linewidths=1.1,zorder=5)
