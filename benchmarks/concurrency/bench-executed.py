...
def create(name,cores,cpus,memory,command):
 docker('run','-d','--name',name,'--network',PREFIX,'--cpuset-cpus',cores,'--cpus',str(cpus),'--memory',memory,'--memory-swap',memory,
        '-v',str(ROOT)+':/bench','-e','GOMAXPROCS='+str(cpus),'-e','K6_NO_USAGE_REPORT=true',IMAGE,*command)
...
parser.add_argument('--out',default='pilot');parser.add_argument('--tools',nargs='+',default=['swarmgo','k6']);parser.add_argument('--swarmgo-binary',default='swarmgo-improved');args=parser.parse_args()
out=ROOT/'results'/args.out;out.mkdir(parents=True,exist_ok=True)
...
manifest={'swarmgo_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT.parent/'SwarmGo',text=True).strip(),
          'generator_cpus':'0-1','generator_cpu_quota':2,'generator_memory_bytes':2*1024**3,'target_cpus':'2-5','target_cpu_quota':4,'target_memory_bytes':2*1024**3,
          'image':IMAGE,'requests':args.requests,'repeats':args.repeats,'workloads':[{'method':'POST','delay_ms':200,'concurrency':10000}],
          'k6_version':'2.3.0','swarmgo_binary':args.swarmgo_binary,'go_build_info':{name:subprocess.check_output(['go','version','-m',str(ROOT/'bin'/(args.swarmgo_binary if name=='swarmgo' else name))],text=True) for name in ['swarmgo','k6','target']},
          ...
          'binary_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (ROOT/'bin').iterdir() if p.name in ['swarmgo','swarmgo-improved','k6','target']}}
try:
